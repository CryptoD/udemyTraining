...
if (MAX_DISPLAY_SPECIAL_PRODUCTS > 0 ) {
  ...
  if ($num_products_count) {
    ...
    while (!$specials->EOF) {
		...
		$products_img = '<a href="' . zen_href_link(zen_get_info_page($specials->fields['products_id']), 'cPath=' . zen_get_generated_category_path_rev($specials->fields["master_categories_id"]) . '&products_id=' . $specials->fields['products_id']) . '">' . (($specials->fields['products_image'] == '' and PRODUCTS_IMAGE_NO_IMAGE_STATUS == 0) ? '' : zen_image(DIR_WS_IMAGES . $specials->fields['products_image'], $specials->fields['products_name'], IMAGE_PRODUCT_NEW_LISTING_WIDTH, IMAGE_PRODUCT_NEW_LISTING_HEIGHT) . '</a>');
		
		$products_name = '<a class="name" href="' . zen_href_link(zen_get_info_page($specials->fields['products_id']), 'cPath=' . zen_get_generated_category_path_rev($specials->fields["master_categories_id"]) . '&products_id=' . $specials->fields['products_id']) . '">' . substr(strip_tags($specials->fields['products_name']), 0, 52) . '</a>';
    ...
    $list_box_contents[$row][$col] = array('params' =>'class="specialsListBoxContents centeredContent back ' . $tm_param . '"',
    'text' => 
